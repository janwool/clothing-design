...
test('loads the Design Studio runtime and material library only after intent', () => {
  ...
  assert.match(template, /src: '\/js\/model-designer\.js\?v=20260924-single-scene-v72'/);
...
test('keeps ordinary detail-page navigation out of the editor', () => {
  ...
  assert.match(template, /if \(window\.location\.hash === '#design'\)/);
...
test('applies a saved project to the detail viewer without opening the editor', () => {
  ...
  assert.match(loadProjectBlock, /else if \(saved\.textureUrl\) await restoreLegacyAppearanceFromTexture\(saved\.textureUrl\)/);
...
test('waits for the detail viewer to receive an applied design before closing the editor', () => {
  ...
  assert.match(saveBlock, /if \(!modelDesignerConfig\.userAuthenticated\)/);
  ...
  assert.match(template, /model-designer\.js\?v=20260924-single-scene-v72/);
});
