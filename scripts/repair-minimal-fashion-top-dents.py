...
def apply_smooth(obj: bpy.types.Object, factor: float, iterations: int) -> None:
    if factor <= 0 or iterations <= 0:
        return
    bpy.ops.object.select_all(action="DESELECT")
    bpy.context.view_layer.objects.active = obj
    obj.select_set(True)
    mod = obj.modifiers.new("Dent reduction smooth", "SMOOTH")
    mod.factor = factor
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Repair dents on minimal-fashion-top source mesh without replacing the model.")
    parser.add_argument("input", type=Path)
    parser.add_argument("output", type=Path)
    parser.add_argument("--smooth-factor", type=float, default=0.12)
    ...
    for obj in mesh_objects:
        ...
        apply_smooth(obj, args.smooth_factor, args.smooth_iterations)
    ...
    print(f"smooth_factor={args.smooth_factor}")
    print(f"smooth_iterations={args.smooth_iterations}")
