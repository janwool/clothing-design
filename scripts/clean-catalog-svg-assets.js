...
const version = process.env.SVG_CLEAN_VERSION || '20260814-svg-clean-v1';
...
function queryRemoteModels() {
  const sql = `SELECT id,slug,category,texture_url FROM models_3d WHERE status='active' ORDER BY id ASC`;
  const env = {
    ...process.env,
    CF_API_TOKEN: '',
    CF_ACCOUNT_ID: '',
...
function parsePath(raw, d) {
  ...
  if (points.length < 3) {
    return { raw, d, area: 0, width: 0, height: 0, span: 0, closed: false };
  }
  let cross = 0;
  ...
  return { raw, d, area: Math.abs(cross) / 2, width, height, span: Math.min(width, height), closed };
}
...
function cleanSvg(svgText) {
  ...
  return {
    ...
    removedThin: paths.filter(item => item.span < effectiveMinSpan).length,
...
async function main() {
  ...
  const sql = [
    'BEGIN TRANSACTION;',
    ...models.map(model => `UPDATE models_3d SET texture_url=${sqlString(model.texture_url)}, updated_at=CURRENT_TIMESTAMP WHERE id=${model.id} AND slug=${sqlString(model.slug)};`),
    'COMMIT;',
    '',
  ].join('\n');
