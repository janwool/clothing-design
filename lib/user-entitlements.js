...
function ensureEntitlementTables() {
  if (!readyPromise) {
    readyPromise = (async () => {
      ...
      await db.run('ALTER TABLE user_subscriptions ADD COLUMN provider TEXT').catch(() => {});
      await db.run('ALTER TABLE user_subscriptions ADD COLUMN provider_customer_id TEXT').catch(() => {});
      await db.run('ALTER TABLE user_subscriptions ADD COLUMN provider_subscription_id TEXT').catch(() => {});
      await db.run('ALTER TABLE user_subscriptions ADD COLUMN provider_product_id TEXT').catch(() => {});
