...
const PLACEHOLDER_MODEL_PATTERN = /(?:^|[\s_-])(demo|placeholder|sample|test|untitled)(?:$|[\s_-])/i;
...
function hasIndexableModelIdentity(model = {}) {
  const name = compactPlainText(model.name);
  const slug = compactPlainText(model.slug).toLowerCase();
  return wordCount(name) >= 3
    && /^[a-z0-9]+(?:-[a-z0-9]+)+$/.test(slug)
    && !PLACEHOLDER_MODEL_PATTERN.test(name)
    && !PLACEHOLDER_MODEL_PATTERN.test(slug);
}
...
function shouldIndexModel(model = {}) {
  return hasIndexableModelIdentity(model)
    && !hasAssetTaxonomyMismatch(model)
    && getModelSeoScore(model) >= 5;
}
