...
function parseImageDataUrl(dataUrl, label = 'Image') {
  ...
  if (!bytes.length || bytes.length > MAX_IMAGE_BYTES) {
    const error = new Error(`${label} must be no larger than ${MAX_IMAGE_BYTES / 1024 / 1024} MB`);
    error.status = 400;
    throw error;
  }
...
async function uploadBinary(bytes, { key, contentType, maxBytes = 25 * 1024 * 1024 }) {
  const data = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes || []);
  if (!data.length || data.length > maxBytes) {
    const error = new Error(`File must be no larger than ${Math.round(maxBytes / 1024 / 1024)} MB`);
    error.status = 400;
    throw error;
  }
  const binding = getObjectStorageBinding();
  if (binding?.put) {
    await binding.put(key, data, { httpMetadata: { contentType } });
    return { key, url: `${getConfig().publicUrl}/${encodeObjectKey(key)}`, size: data.length };
  }
  return { ...(await signedObjectRequest({ method: 'PUT', key, contentType, body: data })), size: data.length };
}
