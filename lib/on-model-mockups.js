const db = require('./db');
const { classifyFashionMockupAsset } = require('./fashion-mockup-categories');
...
function withPublicAssetUrls(record) {
  ...
  const result = {
    ...record,
    fashion_category_slug: classifyFashionMockupAsset(record),
...
async function listOnModelMockupAssets({ garmentType = '', fashionCategory = '', page = 1, pageSize = 30 } = {}) {
  await ensureOnModelMockupTable();
  ...
  const matchingAssets = await db.all(`
    SELECT
      a.*,
    ...
    ORDER BY
      ...
      a.asset_name ASC
  `, params);
  const filteredAssets = fashionCategory
    ? matchingAssets.filter(asset => classifyFashionMockupAsset(asset) === fashionCategory)
    : matchingAssets;
  ...
  const assets = filteredAssets.slice(offset, offset + normalizedPageSize);
...
async function getOnModelMockupAssetSummary() {
  await ensureOnModelMockupTable();
  const [categoryRows, categoryAssets, totals] = await Promise.all([
    db.all(`
    ...
    `),
    db.all(`
      SELECT asset_name, garment_type, title
      FROM on_model_mockup_assets
      WHERE status = 'active'
    `),
    db.get(`
  ...
  const counts = Object.fromEntries(categoryRows.map(row => [row.garment_type, Number(row.count || 0)]));
